...
namespace ProjectX.SlideoutNavigation
{
	public class SlideoutNavigationController : UIViewController
	{
		...
		nfloat _panTranslationX;
		nfloat _slideHandleHeight;
		nfloat _menuWidth;
		SlideHandle _slideHandle;
		...
		public UIBarButtonItem RightNavigationItem { get; set; }
		...
		public SlideHandle SlideHandle {
			...
			set {
				_slideHandle = value;
				switch (value) {
					case SlideHandle.None:
						_slideHandleHeight = 0;
						break;
					case SlideHandle.NavigationBar:
						_slideHandleHeight = 44f + 20f;
						break;
					case SlideHandle.Full:
		...
		public UIViewController MenuViewController {
			...
			set { 
				...
				if (NavigationController != null) {
					//Debug.WriteLine("OOOOOKKKKKKKKKK");
				}
			}
		}
		...
		public SlideoutNavigationController()
		{
			...
			MenuWidth = UIScreen.MainScreen.Bounds.Width - 55;
			ShadowEnabled = true;
			ContainerView = new UIView();
		}

		public SlideoutNavigationController(UIBarButtonItem right)
			: this()
		{
			RightNavigationItem = right;
		}
		...
		public override void ViewDidLoad()
		{
			...
			if (ShadowEnabled) {
				ContainerView.Layer.ShadowOffset = new CGSize(-2, 0);
		...
		void Animate(UIView menuView, UIView mainView, nfloat percentage)
		{
			if (percentage > 1)
				percentage = 1;
		...
		void Pan(UIView view)
		{
			if (_panGesture.State == UIGestureRecognizerState.Began) {
				...
				UIView.Animate(0.01f, 0, 
					UIViewAnimationOptions.BeginFromCurrentState | 
					UIViewAnimationOptions.AllowUserInteraction, animation, EmptyAction);
		...
		void Open(bool animated, nfloat animationTime)
		{
			if (IsOpen)
				return;
		...
		void Close(bool animated, nfloat animationTime)
		{
			if (!IsOpen)
				return;
		...
		///</summary>
		class PanDelegate : UIGestureRecognizerDelegate
		{
			readonly SlideoutNavigationController _controller;
	...
	static public class UIColorExtensions
	{
		static public UIColor FromHex(this UIColor color, int hexValue)
		{
